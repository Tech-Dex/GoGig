...
def embed_help():
    ...
    embed.set_image(url='https://media.discordapp.net/attachments/811254175660638258/813764726160228352/logo.png')
    embed.set_footer(text=f'GoGIG Bot '
                          f'| {time.strftime("%a %b %d, %Y at %H:%M:%S", time.gmtime(time.time()))}')
    embed.add_field(name=f'_snooze', value=f'Turn on/off option of sending logs about errors.\n'
                                           f'Type $sudo_snooze command for more info.', inline=False)
